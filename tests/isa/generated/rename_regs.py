...
def rename_registers(inst_name):
    # Using relative path assuming script runs in the generated folder
    input_file = f"rv32ui-p-{inst_name}.dump"
    output_file = f"rv32ui-p-{inst_name}_rename.dump"

    if not os.path.exists(input_file):
        print(f"Error: Could not find file {input_file} in current directory.")
        sys.exit(1)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python rename_regs.py <instruction_name>")
        print("Example: python rename_regs.py sll")
        sys.exit(1)
    
    inst = sys.argv[1]
    rename_registers(inst)
