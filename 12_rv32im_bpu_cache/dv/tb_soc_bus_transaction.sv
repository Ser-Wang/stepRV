...
wire [31:0] rsp_data;
wire imem_en, imem_we, dcache_req_vld, dcache_req_rdy;
wire dcache_req_load, dcache_req_write, dcache_rsp_rdy, uart_we;
reg dcache_rsp_vld;
...
assign dcache_req_rdy = 1'b1;

always @(posedge clk or negedge rst_n) begin
    if (!rst_n)
        dcache_rsp_vld <= 1'b0;
    else if (!dcache_rsp_vld || dcache_rsp_rdy)
        dcache_rsp_vld <= dcache_req_vld && dcache_req_rdy;
end
...
soc_bus u_bus (
    ...
    .o_dcache_req_vld(dcache_req_vld), .i_dcache_req_rdy(dcache_req_rdy),
    .o_dcache_req_addr(unused_addr1), .o_dcache_req_load(dcache_req_load),
    .o_dcache_req_write(dcache_req_write), .o_dcache_req_wmask(unused_mask1),
    .o_dcache_req_wdata(unused_data1), .i_dcache_rsp_vld(dcache_rsp_vld),
    .o_dcache_rsp_rdy(dcache_rsp_rdy), .i_dcache_rsp_data(32'h1234_5678),
    .o_imem_p1_en(imem_en), .o_imem_p1_we(imem_we), .o_imem_p1_addr(unused_addr0),
    .o_imem_p1_wmask(unused_mask0), .o_imem_p1_wdata(unused_data0),
    .i_imem_p1_rdata(32'haaaa_5555), .o_uart_addr(unused_addr2),
...
bind soc_bus sva_soc_bus u_sva_soc_bus (
    ...
    .mem_req_addr(i_mem_addr), .imem_write(o_imem_p1_we),
...
always @(posedge clk) if (rst_n) begin
    ...
    if (imem_we || (dcache_req_vld && dcache_req_rdy && dcache_req_write) || uart_we)
...
initial begin
    ...
    wait(rsp_vld); #1;
    check(target_write_count==1,
          "D-Cache store route must create one write transaction and a held completion response");
    for(n=0;n<3;n=n+1) begin
        @(posedge clk); #1;
        check(rsp_vld && rsp_data==32'h1234_5678 && !dcache_req_vld,
              "store response must hold without repeating the target write");
    end
    consume_response();

    request(32'hf000_0000, 1'b1, 1'b0);
    check(rsp_vld && rsp_data==0 && !imem_en && !dcache_req_vld,
